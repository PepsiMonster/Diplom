...
def _auto_zoom_ylim(
    ...
    *,
    ...
    """
    Умный выбор границ оси Y.

    Если различия маленькие, не держим ось от нуля,
    чтобы не получать "сплошной одинаковый забор".
    """
...
def _format_metric_axis(ax: plt.Axes, values: np.ndarray) -> None:
    """
    Подбирает формат оси Y под масштаб метрики.
    """
...
def _baseline_percent_delta(values: np.ndarray) -> np.ndarray:
    """
    Строит отклонение от первого сценария в процентах:
    ((x_i / x_0) - 1) * 100
    """
    base = float(values[0])
    if abs(base) <= 1e-14:
...
def plot_metric_comparison(
    ...
    *,
    ...
    """
    Столбчатый график средних значений + доверительные интервалы.
    """
...
def plot_metric_comparison_delta(
    ...
    *,
    ...
    """
    График отклонения средней метрики от baseline в процентах.
    Первый сценарий считается baseline.
    """
    ...
    ax.set_title(f"{metric_name}: отклонение от baseline")
    ax.grid(axis="y", alpha=0.25)
