...
def plot_metric_comparison(
    suite_data: dict[str, Any], metric_name: str, output_dir: Path, dpi: int
) -> Path:
...
def plot_stationary_distribution(
    suite_data: dict[str, Any], output_dir: Path, dpi: int
) -> Path:
    fig, ax = plt.subplots(figsize=(12, 6))
...
def plot_pi_hat_heatmap(suite_data: dict[str, Any], output_dir: Path, dpi: int) -> Path:
    labels = list(suite_data["scenario_results"].keys())
    state_keys: set[int] = set()
    for scenario in suite_data["scenario_results"].values():
        for metric_name in scenario.get("metric_summaries", {}).keys():
...
def generate_standard_plots(
    ...
    for metric in default_metrics:
        if metric in available:
            created.append(plot_metric_comparison(suite_data, metric, output_dir, dpi))
            try:
                created.append(plot_metric_boxplot(suite_data, metric, output_dir, dpi))
            except Exception:
                pass

    if any(name.startswith("pi_hat_") for name in available):
        created.append(plot_stationary_distribution(suite_data, output_dir, dpi))
        created.append(plot_pi_hat_heatmap(suite_data, output_dir, dpi))

    if {"rejected_capacity", "rejected_server", "rejected_resource"}.issubset(available):
        created.append(plot_rejection_breakdown(suite_data, output_dir, dpi))
...
def main() -> None:
    ...
    created = generate_standard_plots(suite_data, output_dir, args.metrics, dpi=args.dpi)
