...
def validate_experiment_values(values_module: ModuleType) -> None:
    ...
    _require(
        _is_nonnegative_int(values_module.ANIMATION_LOG_MAX_JOBS),
        f"ANIMATION_LOG_MAX_JOBS должен быть целым >= 0, получено: {values_module.ANIMATION_LOG_MAX_JOBS!r}",
    )
    _require(
        _is_positive_int(values_module.FULL_RUN_RESULTS_PER_SCENARIO),
        (
            "FULL_RUN_RESULTS_PER_SCENARIO должен быть целым > 0, "
            f"получено: {values_module.FULL_RUN_RESULTS_PER_SCENARIO!r}"
        ),
    )
    ...
    if "hyperexp_2" in values_module.ARRIVAL_PROCESS_FAMILY:
        _require(
            values_module.ARRIVAL_NORMAL_VALUE > 0.0,
            (
                "Для arrival HyperExp(2) требуется ARRIVAL_NORMAL_VALUE > 0, "
                f"получено: {values_module.ARRIVAL_NORMAL_VALUE!r}"
            ),
        )
        arrival_mean_interarrival = 1.0 / values_module.ARRIVAL_NORMAL_VALUE
        arrival_rate_1 = (
            values_module.ARRIVAL_HYPEREXP_FAST_MULTIPLIER
            * values_module.ARRIVAL_NORMAL_VALUE
        )
        arrival_denominator = (
            arrival_mean_interarrival
            - values_module.ARRIVAL_HYPEREXP_P / arrival_rate_1
        )
        _require(
